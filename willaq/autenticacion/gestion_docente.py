...
def verificar_sesion(notificar=None) -> dict:
    """Comprueba si se puede entrar al portal con lo que hay guardado.

    Devuelve {"estado": "activa"|"sin_credenciales"|"credenciales"|"error"}.
    Se usa al abrir el panel: como aquí no se conserva ninguna sesión, lo
    que se comprueba es que las credenciales guardadas sigan sirviendo.
    Mientras MOSTRAR_NAVEGADOR_AL_PROBAR esté en True, esta comprobación
    abre una ventana a la vista.
    """
    notificar = notificar or print

    guardadas = credenciales.cargar()
    if guardadas is None:
        notificar("No hay usuario ni contraseña guardados para Gestión Docente.")
        return {"estado": "sin_credenciales"}

    notificar("Comprobando el acceso a Gestión Docente...")
    resultado = probar_credenciales(
        guardadas["usuario"], guardadas["clave"], notificar=notificar
    )
    if resultado.get("estado") == "credenciales":
        # La contraseña cambió o caducó: se olvida para que el panel vuelva
        # a pedirla en vez de reintentar con una que ya no vale (y arriesgar
        # que el portal bloquee la cuenta por intentos fallidos).
        credenciales.olvidar()
        notificar("        Vuelve a escribir tu usuario y contraseña en el panel.")
    return resultado


def abrir_registro_de_notas(notificar=None, marcar_abierta=None, cancelado=None) -> str:
    """Abre la pantalla de registro de notas del portal, lista para trabajar.

    Antes que nada inicia sesión, en la misma ventana que va a quedar
    abierta y con un navegador limpio (sin perfil guardado). Es a propósito:
    así nunca se arrastra una sesión vieja del portal que pueda estar muerta
    sin avisar, que es exactamente lo que hacía fallar el enfoque anterior.

    Devuelve "ok" (se abrió y luego se cerró), "sin_credenciales" (no hay
    usuario/contraseña guardados), "credenciales" (el portal los rechazó) o
    "error".
    """
    notificar = notificar or print
    marcar_abierta = marcar_abierta or (lambda: None)
    ...
    with sync_playwright() as playwright:
        # headless=False a propósito: el objetivo de esta herramienta es
        # justamente darle al docente la ventana abierta y lista.
        ...
        try:
            ...
            if resultado == "credenciales":
                ...
                notificar('        Vuelve a escribirlos con "Iniciar sesión" en el panel.')
                return "credenciales"
            if resultado != "ok":
                return "error"

            # A las notas se llega por el aplicativo "Académico", no por la
            # URL directa: es ese paso el que deja la sesión lista para las
            # pantallas de /Academico/Secure/.
            pagina_academico = _abrir_aplicativo_academico(pagina, notificar)
            if pagina_academico is None:
                notificar('[AVISO] No se pudo entrar a "Académico".')
                notificar("        La ventana queda abierta por si quieres seguir desde ahí.")
                pagina_academico = pagina
            else:
                notificar(f"Abriendo {URL_GESTION_DOCENTE_REGISTRO_NOTAS} ...")
                if not _abrir_registro_de_notas_en_pagina(pagina_academico, notificar):
                    notificar("[AVISO] El portal no dejó abrir la pantalla de notas.")
                    notificar("        La ventana queda abierta por si quieres seguir desde ahí.")
                else:
                    notificar("[OK] Pantalla de notas abierta. Trabájala en la ventana del navegador.")
                    notificar("     Cuando termines, cierra esa ventana (o usa 'Cerrar' en el panel).")
            marcar_abierta()

            # Se espera indefinidamente, porque el docente decide cuánto
            # tiempo necesita ahí; solo se corta si cierra la ventana o si
            # lo pide desde el panel.
            while True:
                if _ventana_cerrada(contexto):
                    notificar("Se cerró la ventana del navegador.")
                    break
                if cancelado is not None and cancelado():
                    notificar("Cerrando la ventana desde el panel...")
                    break
                time.sleep(SEGUNDOS_ENTRE_SONDEOS)

            return "ok"
        except Exception as error:
            notificar(f"[ERROR] Ocurrió un problema con la pantalla de notas: {error}")
            return "error"
        finally:
            try:
                navegador.close()
